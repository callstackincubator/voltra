...
export default function AndroidDynamicWidgetScreen() {
  const router = useRouter()
  const [headline, setHeadline] = useState('Hello from Android')
  const [unreadCount, setUnreadCount] = useState('3')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [feedback, setFeedback] = useState<Feedback | null>(null)
  const [lastSubmitted, setLastSubmitted] = useState<AndroidDynamicWidgetProps | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    const props: AndroidDynamicWidgetProps = {
    ...
    try {
      await updateAndroidDynamicWidget(DYNAMIC_WIDGET_ID, props)
      setLastSubmitted(props)
      setFeedback({ kind: 'success', message: 'Android Dynamic Widget updated successfully.' })
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      setFeedback({ kind: 'error', message: `Unable to update the Android Dynamic Widget: ${message}` })
    } finally {
  ...
  return (
    <ScreenLayout
      title="Android Dynamic Widget"
      description="Send runtime props to the entry-based Android Dynamic Widget. Pin the Client Demo widget first to see updates on your home screen."
    >
      <Card>
        ...
        <View style={styles.field}>
          ...
          <TextInput
            accessibilityLabel="Android Dynamic Widget headline"
            style={styles.input}
            value={headline}
            onChangeText={setHeadline}
            editable={!isSubmitting}
            maxLength={48}
            placeholder="e.g. Android inbox"
        ...
        <View style={styles.field}>
          ...
          <TextInput
            accessibilityLabel="Android Dynamic Widget unread count"
            style={styles.input}
            value={unreadCount}
        ...
        <View style={styles.buttonContainer}>
          <Button
            title={isSubmitting ? 'Updating...' : 'Update Android Dynamic Widget'}
      ...
      <View style={styles.footer}>
        <Button title="Back to Android Widgets" variant="ghost" onPress={() => router.back()} />
      </View>
    </ScreenLayout>
  )
}
