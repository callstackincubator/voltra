/* eslint-disable */
// 🤖 AUTO-GENERATED from data/components.json
// DO NOT EDIT MANUALLY - Changes will be overwritten
// Schema version: 1.0.0

/**
 * Unified mapping from full names to short names
 * Used for props and style properties
 */
export const NAME_TO_SHORT: Record<string, string> = {
  absolutePosition: 'ap',
  adjustsFontSizeToFit: 'afsf',
  alignItems: 'ai',
  alignment: 'al',
  alignSelf: 'as',
  allowFontScaling: 'afs',
  aspectRatio: 'ar',
  assetName: 'an',
  background: 'bkg',
  backgroundColor: 'bg',
  backgroundStyle: 'bgs',
  base64: 'b64',
  border: 'bd',
  borderColor: 'bc',
  borderRadius: 'br',
  borderWidth: 'bw',
  bottom: 'b',
  buttonStyle: 'bs',
  checked: 'chk',
  clipped: 'clip',
  color: 'c',
  colors: 'cls',
  contentAlignment: 'ca',
  contentColor: 'cc',
  contentDescription: 'cdesc',
  cornerRadius: 'cr',
  countDown: 'cd',
  currentValueLabel: 'cvl',
  deepLinkUrl: 'dlu',
  defaultValue: 'dv',
  destination: 'dest',
  direction: 'dir',
  dither: 'dth',
  durationMs: 'dur',
  enabled: 'en',
  endAtMs: 'end',
  endPoint: 'ep',
  fallback: 'flb',
  fillMaxHeight: 'fmh',
  fillMaxWidth: 'fmw',
  fixedSizeHorizontal: 'fsh',
  fixedSizeVertical: 'fsv',
  flex: 'fl',
  flexBasis: 'fb',
  flexDirection: 'fdir',
  flexGrow: 'fg',
  flexGrowWidth: 'fgw',
  flexShrink: 'fk',
  font: 'fnt',
  fontFamily: 'ff',
  fontSize: 'fs',
  fontVariant: 'fvar',
  fontWeight: 'fw',
  foregroundStyle: 'fgs',
  frame: 'f',
  gap: 'g',
  gaugeStyle: 'gs',
  glassEffect: 'ge',
  height: 'h',
  horizontalAlignment: 'halig',
  horizontalPadding: 'hp',
  icon: 'ic',
  id: 'id',
  italic: 'it',
  justifyContent: 'jc',
  kerning: 'kern',
  label: 'lbl',
  layout: 'ly',
  layoutPriority: 'lp',
  left: 'l',
  letterSpacing: 'ls',
  lineHeight: 'lh',
  lineLimit: 'll',
  lineSpacing: 'lsp',
  lineWidth: 'lw',
  margin: 'm',
  marginBottom: 'mb',
  marginHorizontal: 'mh',
  marginLeft: 'ml',
  marginRight: 'mr',
  marginTop: 'mt',
  marginVertical: 'mv',
  maskElement: 'me',
  maxFontSizeMultiplier: 'mfsm',
  maxHeight: 'maxh',
  maximumValue: 'max',
  maximumValueLabel: 'maxl',
  maxLines: 'mxl',
  maxWidth: 'maxw',
  minHeight: 'minh',
  minimumFontScale: 'mfs',
  minimumValue: 'min',
  minimumValueLabel: 'minvl',
  minLength: 'minl',
  minWidth: 'minw',
  monospacedDigit: 'md',
  multilineTextAlignment: 'mta',
  name: 'n',
  numberOfLines: 'nol',
  offset: 'off',
  offsetX: 'ox',
  offsetY: 'oy',
  opacity: 'op',
  overflow: 'ov',
  padding: 'pad',
  paddingBottom: 'pb',
  paddingHorizontal: 'ph',
  paddingLeft: 'pl',
  paddingRight: 'pr',
  paddingTop: 'pt',
  paddingVertical: 'pv',
  position: 'pos',
  progressColor: 'pc',
  resizeMode: 'rm',
  right: 'r',
  rotationEffect: 're',
  scale: 'sc',
  scaleEffect: 'sce',
  shadow: 'sh',
  shadowColor: 'shc',
  shadowOffset: 'sho',
  shadowOpacity: 'shop',
  shadowRadius: 'shr',
  showHours: 'shrs',
  size: 'sz',
  smallCaps: 'smc',
  source: 'src',
  spacing: 'sp',
  startAtMs: 'start',
  startPoint: 'stp',
  stops: 'sts',
  strikethrough: 'st',
  style: 's',
  systemImage: 'si',
  text: 'txt',
  textAlign: 'ta',
  textDecorationLine: 'tdl',
  textStyle: 'ts',
  textTemplates: 'tt',
  thumb: 'th',
  tint: 'tnt',
  tintColor: 'tc',
  title: 'ttl',
  top: 't',
  trackColor: 'trc',
  transform: 'tf',
  type: 'typ',
  underline: 'ul',
  value: 'v',
  verticalAlignment: 'valig',
  weight: 'wt',
  width: 'w',
  zIndex: 'zi',
}

/**
 * Reverse mapping from short names to full names
 */
export const SHORT_TO_NAME: Record<string, string> = {
  ap: 'absolutePosition',
  afsf: 'adjustsFontSizeToFit',
  ai: 'alignItems',
  al: 'alignment',
  as: 'alignSelf',
  afs: 'allowFontScaling',
  ar: 'aspectRatio',
  an: 'assetName',
  bkg: 'background',
  bg: 'backgroundColor',
  bgs: 'backgroundStyle',
  b64: 'base64',
  bd: 'border',
  bc: 'borderColor',
  br: 'borderRadius',
  bw: 'borderWidth',
  b: 'bottom',
  bs: 'buttonStyle',
  chk: 'checked',
  clip: 'clipped',
  c: 'color',
  cls: 'colors',
  ca: 'contentAlignment',
  cc: 'contentColor',
  cdesc: 'contentDescription',
  cr: 'cornerRadius',
  cd: 'countDown',
  cvl: 'currentValueLabel',
  dlu: 'deepLinkUrl',
  dv: 'defaultValue',
  dest: 'destination',
  dir: 'direction',
  dth: 'dither',
  dur: 'durationMs',
  en: 'enabled',
  end: 'endAtMs',
  ep: 'endPoint',
  flb: 'fallback',
  fmh: 'fillMaxHeight',
  fmw: 'fillMaxWidth',
  fsh: 'fixedSizeHorizontal',
  fsv: 'fixedSizeVertical',
  fl: 'flex',
  fb: 'flexBasis',
  fdir: 'flexDirection',
  fg: 'flexGrow',
  fgw: 'flexGrowWidth',
  fk: 'flexShrink',
  fnt: 'font',
  ff: 'fontFamily',
  fs: 'fontSize',
  fvar: 'fontVariant',
  fw: 'fontWeight',
  fgs: 'foregroundStyle',
  f: 'frame',
  g: 'gap',
  gs: 'gaugeStyle',
  ge: 'glassEffect',
  h: 'height',
  halig: 'horizontalAlignment',
  hp: 'horizontalPadding',
  ic: 'icon',
  id: 'id',
  it: 'italic',
  jc: 'justifyContent',
  kern: 'kerning',
  lbl: 'label',
  ly: 'layout',
  lp: 'layoutPriority',
  l: 'left',
  ls: 'letterSpacing',
  lh: 'lineHeight',
  ll: 'lineLimit',
  lsp: 'lineSpacing',
  lw: 'lineWidth',
  m: 'margin',
  mb: 'marginBottom',
  mh: 'marginHorizontal',
  ml: 'marginLeft',
  mr: 'marginRight',
  mt: 'marginTop',
  mv: 'marginVertical',
  me: 'maskElement',
  mfsm: 'maxFontSizeMultiplier',
  maxh: 'maxHeight',
  max: 'maximumValue',
  maxl: 'maximumValueLabel',
  mxl: 'maxLines',
  maxw: 'maxWidth',
  minh: 'minHeight',
  mfs: 'minimumFontScale',
  min: 'minimumValue',
  minvl: 'minimumValueLabel',
  minl: 'minLength',
  minw: 'minWidth',
  md: 'monospacedDigit',
  mta: 'multilineTextAlignment',
  n: 'name',
  nol: 'numberOfLines',
  off: 'offset',
  ox: 'offsetX',
  oy: 'offsetY',
  op: 'opacity',
  ov: 'overflow',
  pad: 'padding',
  pb: 'paddingBottom',
  ph: 'paddingHorizontal',
  pl: 'paddingLeft',
  pr: 'paddingRight',
  pt: 'paddingTop',
  pv: 'paddingVertical',
  pos: 'position',
  pc: 'progressColor',
  rm: 'resizeMode',
  r: 'right',
  re: 'rotationEffect',
  sc: 'scale',
  sce: 'scaleEffect',
  sh: 'shadow',
  shc: 'shadowColor',
  sho: 'shadowOffset',
  shop: 'shadowOpacity',
  shr: 'shadowRadius',
  shrs: 'showHours',
  sz: 'size',
  smc: 'smallCaps',
  src: 'source',
  sp: 'spacing',
  start: 'startAtMs',
  stp: 'startPoint',
  sts: 'stops',
  st: 'strikethrough',
  s: 'style',
  si: 'systemImage',
  txt: 'text',
  ta: 'textAlign',
  tdl: 'textDecorationLine',
  ts: 'textStyle',
  tt: 'textTemplates',
  th: 'thumb',
  tnt: 'tint',
  tc: 'tintColor',
  ttl: 'title',
  t: 'top',
  trc: 'trackColor',
  tf: 'transform',
  typ: 'type',
  ul: 'underline',
  v: 'value',
  valig: 'verticalAlignment',
  wt: 'weight',
  w: 'width',
  zi: 'zIndex',
}

/**
 * Shorten a name using the unified mapping
 * Returns the original name if no short form exists
 */
export function shorten(name: string): string {
  return NAME_TO_SHORT[name] ?? name
}

/**
 * Expand a short name back to the full name
 * Returns the original value if no expansion exists
 */
export function expand(short: string): string {
  return SHORT_TO_NAME[short] ?? short
}
