...
describe('Chart serialization', () => {
  test('does not serialize chartScrollableAxes', () => {
    const output = renderVoltraVariantToJson(
      <Voltra.Chart {...({ chartScrollableAxes: 'horizontal' } as any)}>
        <Voltra.BarMark data={[{ x: 'Jan', y: 10 }]} />
      </Voltra.Chart>
    )
  ...
  test('BarMark serializes grouped stacking only', () => {
    const groupedOutput = renderVoltraVariantToJson(
      <Voltra.Chart>
        <Voltra.BarMark data={[{ x: 'Jan', y: 10, series: 'A' }]} stacking="grouped" />
      </Voltra.Chart>
    )
    ...
    const defaultOutput = renderVoltraVariantToJson(
      <Voltra.Chart>
        <Voltra.BarMark data={[{ x: 'Jan', y: 10, series: 'A' }]} />
      </Voltra.Chart>
    )
  ...
  test('AreaMark does not serialize stacking', () => {
    const output = renderVoltraVariantToJson(
      <Voltra.Chart>
        <Voltra.AreaMark data={[{ x: 'Jan', y: 10, series: 'A' }]} />
      </Voltra.Chart>
    )
  ...
  test('RuleMark serializes both x and y values when both are provided', () => {
    const output = renderVoltraVariantToJson(
      <Voltra.Chart>
        <Voltra.RuleMark xValue="Jan" yValue={75} />
      </Voltra.Chart>
    )
