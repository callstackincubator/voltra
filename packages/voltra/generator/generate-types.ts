...
const TS_ANDROID_PACKAGE_PAYLOAD_OUTPUT_DIR = path.join(ROOT_DIR, '..', 'android', 'src', 'payload')
...
const main = () => {
  ...
  console.log(
    ...
    } files in ${fromRoot(TS_PROPS_OUTPUT_DIR)}/ and ${fromRoot(TS_IOS_PROPS_OUTPUT_DIR)}/`
  )
  console.log(
  ...
  console.log(
    `   Short names: ${Object.keys(tsShortNameFiles).length} TypeScript files in ${fromRoot(
      TS_CORE_PAYLOAD_OUTPUT_DIR
    )}/, ${Object.keys(swiftShortNameFiles).length} Swift files, ${
      Object.keys(kotlinShortNameFiles).length
    } Kotlin files`
  )
  console.log()
  ...
  console.log(
    `   2. Create component files manually in ${fromRoot(TS_PROPS_OUTPUT_DIR)}/, ${fromRoot(
      TS_IOS_PROPS_OUTPUT_DIR
    )}/, and ${fromRoot(TS_ANDROID_PROPS_OUTPUT_DIR)}/ using createVoltraComponent`
  )
