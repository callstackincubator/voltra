...
export async function setWidgetServerUpdate(
  ...
  return getNativeVoltra().setWidgetServerUpdate(JSON.stringify(settings ?? {}), options?.widgetId ?? null)
}
...
export async function getWidgetServerUpdate(
  ...
  const json = await getNativeVoltra().getWidgetServerUpdate(options?.widgetId ?? null)
