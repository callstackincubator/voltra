...
function createGeneratedEntry(projectRoot: string, generatedRoot: string, definition: RegisteredVoltraLiveActivity) {
  ...
  const content = [
    ...
    'if (typeof globalThis.__voltraDynamicLiveActivityDefinitionUpdated === "function") {',
    `  globalThis.__voltraDynamicLiveActivityDefinitionUpdated(${JSON.stringify(definition.id)})`,
    '}',
    '',
