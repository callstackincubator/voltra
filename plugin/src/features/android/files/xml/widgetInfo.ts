...
export function generateWidgetInfoXml(widget: AndroidWidgetConfig): string {
  const minWidth = widget.minWidth || 110
  const minHeight = widget.minHeight || 110
  const targetCellWidth = widget.targetCellWidth || 2
  const targetCellHeight = widget.targetCellHeight || 2
