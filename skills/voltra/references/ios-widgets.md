...
- Use `Voltra` from `@use-voltra/ios` and widget APIs such as `updateWidget`, `scheduleWidget`, `reloadWidgets`, `clearWidget`, `clearAllWidgets`, `getActiveWidgets`, and `VoltraWidgetPreview` from `@use-voltra/ios-client`.
...
- If the task includes `serverUpdate`, widget polling intervals, widget auth credentials, or `createIOSWidgetUpdateHandler`, also read `server-driven-widgets.md`.
