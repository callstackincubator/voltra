...
class VoltraView: ExpoView {
  ...
  private var components: [VoltraComponent] = []
  ...
  private func setupHostingController() {
    let view = Voltra(components: [], callback: { _ in }, activityId: viewId)
  ...
  private func parseAndUpdatePayload(_ jsonString: String) {
    guard let data = jsonString.data(using: .utf8) else {
      print("Error setting payload in VoltraView: Invalid UTF-8 string")
      components = []
      updateView()
      return
    }

    // Try parsing as array first (most common case), then as single component
    do {
      components = try JSONDecoder().decode([VoltraComponent].self, from: data)
    } catch {
      do {
        let component = try JSONDecoder().decode(VoltraComponent.self, from: data)
  ...
  private func updateView() {
    ...
    let newView = Voltra(components: components, callback: { _ in }, activityId: viewId)
